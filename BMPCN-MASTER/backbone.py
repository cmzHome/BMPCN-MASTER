...
class ConvNet(nn.Module):
    ...
    def forward(self, input_data):
        ...
        output_data = self.conv_4(out_3)  # [128,512,5,5]
        out_for_word = output_data.view(output_data.shape[0], output_data.shape[1], -1)  # [128,512,25]
        out = self.layer_last(output_data.view(output_data.size(0), -1))
        return out, out_for_word
...
class ResNet12(nn.Module):
    ...
    def forward(self, x):
        ...
        # 256 -> 512
        x = self.layer4(x)
        output_for_word = x.view(x.shape[0], x.shape[1], -1)
        output_data = self.avgpool(x)
        output_data = output_data.view(output_data.size(0), -1)
        # 512 -> 128
        output_data = self.layer_last(output_data)       # [128,128]
        out = []
        out.append(output_data)
        out.append(output_for_word)
        # no FC here
        return out
